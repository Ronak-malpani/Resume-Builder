...
const Hero = () => {
    ...
    return (
        <>
            ...
            <div className="pb-8 bg-white overflow-hidden relative">
                {/* Navbar - Fixed Flex Alignment & Horizontal Row */}
                <nav className="z-50 max-w-7xl mx-auto flex items-center justify-between w-full py-5 px-4 sm:px-6 lg:px-8 text-sm">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <button onClick={() => setMenuOpen(true)} className="md:hidden p-2 text-slate-600 focus:outline-none" aria-label="Open Menu">
                ...
                {/* Mobile Menu Overlay */}
                <div className={`fixed inset-0 z-[100] bg-white flex flex-col items-center justify-center text-2xl gap-6 md:hidden transition-all duration-300 px-6 ${menuOpen ? "opacity-100 visible" : "opacity-0 invisible translate-y-10"}`}>
                    <button onClick={() => setMenuOpen(false)} className="absolute top-6 right-8 text-4xl font-light text-slate-400 focus:outline-none" aria-label="Close Menu">✕</button>
                    ...
                    <a href="#testimonials" onClick={() => setMenuOpen(false)} className="font-semibold text-slate-800">Testimonials</a>
                    <a href="#contact" onClick={() => setMenuOpen(false)} className="font-semibold text-slate-800">Contact</a>
    
                    {/* Auth buttons inside mobile drawer */}
                    <div className="w-full max-w-xs flex flex-col gap-3 mt-4">
                        {!user ? (
                            <>
                                <Link to="/app?state=register" onClick={() => setMenuOpen(false)} className="w-full text-center py-3 bg-green-500 text-white font-semibold rounded-full shadow-lg shadow-green-100">
                                    Get Started
                                </Link>
                                <Link to="/app?state=login" onClick={() => setMenuOpen(false)} className="w-full text-center py-3 border border-slate-200 text-slate-700 font-semibold rounded-full">
                                    Login
                                </Link>
                            </>
                        ) : (
                            <Link to="/app" onClick={() => setMenuOpen(false)} className="w-full text-center py-3 bg-green-500 text-white font-semibold rounded-full shadow-lg shadow-green-100">
                                Go to Dashboard
                            </Link>
                        )}
                    </div>
                ...
                {/* Hero Content */}
                <div className="relative flex flex-col items-center justify-center px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
                    {/* Background Blur */}
                    <div className="absolute top-0 -z-10 size-[280px] md:size-[500px] bg-green-100 blur-[120px] opacity-40 rounded-full pointer-events-none"></div>

                    {/* Social Proof */}
                    <div className="flex items-center mt-12 md:mt-20 bg-white/50 backdrop-blur-sm border border-slate-100 py-2 px-4 rounded-full shadow-sm">
                    ...
                    <h1 className="text-3xl sm:text-5xl md:text-7xl font-bold max-w-4xl text-center mt-8 text-slate-900 leading-[1.15] sm:leading-[1.1] tracking-tight">
                            ...
                            <div className="marquee-inner flex will-change-transform min-w-max">
                                {[...companyLogos, ...companyLogos, ...companyLogos].map((company, index) => (
                                    <div key={index} className="mx-10 md:mx-14 flex items-center justify-center h-16">
                                       {/* Replace the <img> tag inside your marquee map loop with this */}
                                            <img 
                                                src={`https://raw.githubusercontent.com/simple-icons/simple-icons/develop/icons/${company}.svg`} 
                                                alt={company} 
                                                className="h-7 w-auto object-contain opacity-80 hover:opacity-100 transition-opacity duration-300 filter grayscale invert-0" 
                                                draggable={false} 
