...
const MinimalImageTemplate = ({ data, accentColor }) => {
  ...
  return (
    <div className="w-[210mm] min-h-[297mm] h-full bg-white text-zinc-800 font-sans shadow-lg print:shadow-none overflow-hidden break-words">
      <div className="grid grid-cols-12 min-h-[297mm] h-full">

        {/* LEFT SIDEBAR */}
        <aside className="col-span-4 border-r border-zinc-200 bg-zinc-50/30 flex flex-col justify-between h-full">
            <div>
                {/* IMAGE */}
                <div className="p-8 pb-4 flex justify-center">
                    {data.personal_info?.image ? (
                        <div className="w-32 h-32 rounded-full overflow-hidden border-4 border-white shadow-xs" style={{ borderColor: accentColor + "20" }}>
                            <img
                                src={
                        ...
                        <div className="space-y-3 text-xs font-medium text-zinc-700">
                            {data.personal_info?.phone && (
                                <div className="flex items-center gap-3">
                                    <span className="p-1.5 rounded-full bg-white shadow-xs text-zinc-500">
                    ...
                    <section>
                        ...
                        <div className="flex flex-wrap gap-2">
                            {data.skills.map((skill, index) => (
                                <span 
                                    key={index} 
                                    className="text-xs px-2 py-1 bg-white border border-zinc-100 rounded shadow-xs text-zinc-700"
                                >
                                    {skill}
                                </span>
                            ))}
        ...
        {/* RIGHT CONTENT */}
        <main className="col-span-8 flex flex-col justify-between h-full">
            <div>
                {/* HEADER NAME */}
